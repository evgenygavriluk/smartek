...
class Biblioteks
{
    ...
    public static function getBibliotekaBookCnt($bId=0, $authorId=0){
        if($bId==0 and $authorId==0) $result = Book::all()->count();
        if($bId==0 and $authorId>0)  $result = Author::find($authorId)->books->count();
        if($bId>0  and $authorId==0) $result = BibliotekaBook::where('biblioteka_id','=',$bId)->count();
    ...
    public static function getContainBooks($bId=0, $curPage=1, $elementsPerPage=5, $authorId=0, $reitingSort=0){
        ...
        if($bId==0 && $authorId==0) {
            $sql = "SELECT b.id, b.bookname, b.bookpublicyear, b.bookimage, b.commentscnt, b.allballs, t.themaname FROM books as b JOIN themas t ON t.id = b.bookthema $sort LIMIT $elementsPerPage OFFSET $rangeStart";
        }
        // Все книги автора authorId
        if($bId==0 && $authorId>0) {
            $sql = "SELECT b.id, b.bookname, b.bookpublicyear, b.bookimage, b.commentscnt, b.allballs, t.themaname FROM books as b JOIN themas t ON t.id = b.bookthema JOIN book_author ba ON ba.book_id = b.id WHERE ba.author_id = $authorId $sort LIMIT $elementsPerPage OFFSET $rangeStart";
        }
        // Книги автора authorId из библиотеки bId
        if($bId>0 && $authorId>0){
            $sql = "SELECT b.id, b.bookname, b.bookpublicyear, b.bookimage, b.commentscnt, b.allballs, t.themaname FROM books as b JOIN biblioteka_book bb ON bb.book_id = b.id JOIN themas t ON t.id = b.bookthema JOIN book_author ba ON ba.book_id = b.id WHERE bb.biblioteka_id = $bId AND ba.author_id = $authorId $sort LIMIT $elementsPerPage OFFSET $rangeStart";
        }
